...
def amortize(principal: float, rate_annual: float, years: int, freq: str, start: date, when: str) -> pd.DataFrame:
    ...
    for k in range(1, n + 1):
        ...
        bal -= principal_pay
        rows.append(
            {"Period": k, "Date": d, "Payment": round(total, 2),
...
def format_duration(n_periods: int, freq: str) -> str:
    ...
    if freq == "Monthly":
        return f"{years_i} years ({n_periods} months)" if rem == 0 else f"{years_i} years {rem} months ({n_periods} months)"
    if freq == "Quarterly":
        return f"{years_i} years ({n_periods} quarters)" if rem == 0 else f"{years_i} years {rem} quarters ({n_periods} quarters)"
...
SINGLE_DEFAULTS = dict(
    name="", age=25, start_date=date.today(),
    price=0.0, deposit=0.0, rate=8.5, years=20,
    freq="Monthly", when_begin=False
)
...
def open_fullscreen_form():
    ...
    # re-seed fs_* from current main state explicitly when user clicks edit
    sync_main_to_form()
...
def sidebar_single_controls():
    # Bind directly to MAIN keys; do NOT pass value=/index=
    with st.sidebar:
        st.header("Inputs")
        ...
        c3, c4 = st.columns(2)
        with c3:
            st.slider("Interest Rate (APR, %)", min_value=0.0, max_value=30.0, step=0.1, key="rate")
        with c4:
            st.slider("Duration (Years)", min_value=1, max_value=40, step=1, key="years")

        st.selectbox("Repayment Frequency", list(PERYEAR.keys()), key="freq")
...
def scenario_form(prefix: str, defaults: dict):
    # Compare mode uses separate dicts; explicit values are fine here
    c = {}
...
def render_pie(title: str, totals: dict, size: float = 5.0, radius: float = 0.7):
    ...
    ax.pie(parts, labels=["Interest", "Principal"], autopct="%1.1f%%", startangle=90, radius=radius)
    ax.axis("equal")
...
# Main flow
# =========================
mode_toggle()  # once at the top

if not st.session_state["submitted"]:
    if not st.session_state["compare_mode"]:
        st.subheader("Enter loan details")

        # Seed fs_* ONLY once per edit session
        ...
        with st.form("single_form", clear_on_submit=False):
            ...
            # Full-screen inputs (NO value=/index=)
            r1c1, r1c2, r1c3 = st.columns([2, 1, 1])
            with r1c1: st.text_input("Your Name", key="fs_name")
            with r1c2: st.number_input("Age", min_value=18, max_value=100, key="fs_age")
            with r1c3: st.date_input("Start Date", key="fs_start_date")

            r2c1, r2c2 = st.columns(2)
            with r2c1: st.number_input("Asset Price / Loan Target", min_value=0.0, step=50_000.0, format="%.2f", key="fs_price")
            with r2c2: st.number_input("Deposit / Down Payment", min_value=0.0, step=10_000.0, format="%.2f", key="fs_deposit")

            r3c1, r3c2 = st.columns(2)
            with r3c1: st.slider("Interest Rate (APR, %)", min_value=0.0, max_value=30.0, step=0.1, key="fs_rate")
            with r3c2: st.slider("Duration (Years)", min_value=1, max_value=40, step=1, key="fs_years")

            r4c1, r4c2 = st.columns(2)
            with r4c1: st.selectbox("Repayment Frequency", list(PERYEAR.keys()), key="fs_freq")
            with r4c2: st.toggle("Pay at beginning (advance)?", key="fs_when_begin")

            if st.form_submit_button("Calculate"):
                ss.update({
                    "name": ss["fs_name"],
                    "age": ss["fs_age"],
                    "start_date": ss["fs_start_date"],
                    "price": ss["fs_price"],
                    "deposit": ss["fs_deposit"],
                    "rate": ss["fs_rate"],
                    "years": ss["fs_years"],
                    "freq": ss["fs_freq"],
                    "when_begin": ss["fs_when_begin"],
                    "submitted": True,
                })
                st.rerun()
        ...
        with st.form("compare_form", clear_on_submit=False):
            ...
            with colA: A = scenario_form("A", st.session_state["A"])
            with colB: B = scenario_form("B", st.session_state["B"])
    ...
    if not st.session_state["compare_mode"]:
        # Compute/render FIRST, then show sidebar for after-submit tweaks
        cfg = dict(
        ...
        render_summary_and_table(
            "Loan", totals, sched, totals["freq"], milestones=ms,
            name=st.session_state["name"], age=st.session_state["age"], start_date=st.session_state["start_date"]
        )

        st.subheader("Chart")
        render_pie("Loan", totals, size=4.5, radius=0.6)

        # Sidebar controls (bound to MAIN keys)
